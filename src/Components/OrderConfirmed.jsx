...
useEffect(() => {
    if (ETA) {
        const interval = setInterval(() => {
            ...
            const CalculateDifference = Math.max(0, Math.round((TimeEta - now) / 60000)); // Calculate the difference in seconds

            setTimeLeft(CalculateDifference); // Update the time left state

            if (CalculateDifference <= 0) clearInterval(interval); // Clear the interval when the time is up
